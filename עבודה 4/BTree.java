...
public class BTree<T extends Comparable<T>> {
    ...
    protected Deque<T> myDeque = new LinkedList<>();
    ...
    public void insert(T value) {
        ...
        if (root == null) {
            ...
            while (currentNode != null && !wasAdded) {
                ...
                if (currentNode.getNumberOfKeys() == maxDegree - 1) {
                    if(root == currentNode) { //split in root, when joined we expect to see null parent
                        myDeque.add(null);
                        myDeque.add(null);
                    }
                    split(currentNode);
                ...
                if (currentNode.isLeaf()) {
                    ...
                    int idx = currentNode.getValuePosition(value);
                    if(idx == 0) { //if goes through the leftmost child in node
                        myDeque.add(null);
                        myDeque.add(currentNode.getKey(idx));
                    }
                    else if(idx == maxDegree-1) { //if goes through the rightmost child in node
                        myDeque.add(currentNode.getKey(idx-1));
                        myDeque.add(null);
                    }
                    else {
                        myDeque.add(currentNode.getKey(idx - 1)); //if goes through middle of 2 keys
                        myDeque.add(currentNode.getKey(idx));
                    }
                    currentNode = currentNode.getChild(idx);
                }
        ...
        myDeque.add(value); //adding the inserted value
        size++;
    }
